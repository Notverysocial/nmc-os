...
function AdminAuth({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "#07090F" }}>
      <div style={{ background: "rgba(15,17,23,0.95)", border: "1px solid rgba(30,58,138,0.3)" }} className="rounded-2xl p-10 w-full max-w-sm">
        <div className="flex items-center gap-2.5 mb-8">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "#1A2B50" }}>
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <AdminAuth>
      <div className="flex min-h-screen" style={{ background: "#07090F", fontFamily: "var(--font-dm-sans)" }}>

        {/* Sidebar — Desktop */}
        <aside
          className="hidden lg:flex flex-col w-60 shrink-0 border-r"
          style={{ background: "#0D1117", borderColor: "rgba(30,58,138,0.2)" }}
        >
            ...
            {navItems.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link
                  ...
                  style={{
                    background: active ? "rgba(26,43,80,0.5)" : "transparent",
                    color: active ? "#ffffff" : "#6b7280",
                    borderLeft: active ? "2px solid #2563EB" : "2px solid transparent",
                  }}
                >
                  <Icon className="w-4 h-4 shrink-0" style={{ color: active ? "#3B82F6" : "#6b7280" }} />
                  {label}
          ...
          <div className="px-4 py-4 border-t space-y-3" style={{ borderColor: "rgba(30,58,138,0.2)" }}>
            {/* Agent status */}
            <div className="rounded-xl p-3" style={{ background: "rgba(26,43,80,0.2)", border: "1px solid rgba(37,99,235,0.15)" }}>
            ...
            <aside className="relative flex flex-col w-64 h-full border-r" style={{ background: "#0D1117", borderColor: "rgba(30,58,138,0.2)" }}>
              <div className="px-5 py-5 flex items-center justify-between border-b" style={{ borderColor: "rgba(30,58,138,0.2)" }}>
                <div className="flex items-center gap-2.5">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "#1A2B50" }}>
